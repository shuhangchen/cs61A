        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'container') and self.ant.container:
                    ...
                    if not (self.ant.name is 'Queen' and not self.ant.is_impostor):
                        self.ant = None
                    else:
                        return
            else:
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if not (self.ant.ant.name is 'Queen' and not self.ant.ant.is_impostor):
                        self.ant.ant = None
                    else:
                        return
                else:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None:
            return False
        elif not self.place.ant.block_path:
            return False
        else:
            return True
        # END Problem 6A
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container and (not other.container):
            if self.ant is None:
                return True
            else:
                return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        ranges = 0
        while place.entrance:
            if place is not hive:
                bee = random_or_none(place.bees)
                if bee:
                    if ranges < self.min_range:
                        place = place.entrance
                    elif ranges > self.max_range:
                        return None
                    else:
                        return bee
                else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            bees_in_here_copy = self.place.bees[:]
            for bee in bees_in_here_copy:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            bee_to_be_eaten = random_or_none(self.place.bees)
            if bee_to_be_eaten is not None:
                bee_to_be_eaten.reduce_armor(bee_to_be_eaten.armor)
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 8
        bees_here_copy = self.place.bees[:]
        for bee in bees_here_copy:
            bee.reduce_armor(1)
        ...
        if self.is_impostor:
            ...
            while place is not None:
                if place.ant is not None:
                    if hasattr(place.ant, 'ant') and place.ant.ant is not None:
                        if place.ant.ant not in self.doubled_ants:
                            place.ant.ant.damage *= 2
                            self.doubled_ants = self.doubled_ants + [place.ant.ant]
                    if place.ant not in self.doubled_ants:
                        self.doubled_ants = self.doubled_ants + [place.ant]
                        place.ant.damage *= 2
                place = place.exit
        # END Problem 9
