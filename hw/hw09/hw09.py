...
class Link:
    ...
    def get_ith_link(self, i):
        if i == 0:
            return self
        else:
            return self.rest.get_ith_link(i-1)
...
def nearest_two(x):
    ...
    if abs(cand1 - x) > abs(cand2 - x):
        power_of_two = cand2
    elif abs(cand1 - x) < abs(cand2 - x):
        power_of_two = cand1
    else:
        power_of_two = max(cand1, cand2)
    return power_of_two
# Q2
...
def n_fold_smooth(f, dx, n):
    ...
    if n==1:
        ...
        return smooth(n_fold_smooth(f, dx, n-1), dx)
...
def near_golden(perimeter):
    ...
    """
    "*** YOUR CODE HERE ***"
    def compute_dis(height, width):
        return abs(height/width - width/height +1)
    mid = perimeter //2
    height = floor(mid/2)
    width = mid - height
    height_best = height
    initial = compute_dis(height, width)
    dis_best = initial
    while height > 0 :
        distance = compute_dis(height, width)
...
def is_circular(G):
    ...
    for v in G:
        if reaches_circularity(G, v, len(G)):
            return True
    return False
def reaches_circularity(G, v0, size):
...
def intersection(xs, ys):
    ...
    if xs is Link.empty or ys is Link.empty:
        ...
        return intersection(xs.get_ith_link(len(xs)-len(ys)), ys)
    elif len(xs) < len(ys):
        return intersection(xs, ys.get_ith_link(len(ys) - len(xs)))
    else:
        if xs is ys:
            return xs
        else:
    ...
    def advanced_counter():
        ...
        def counter(content):
            ...
            if content == 'count':
                count = count + 1
                return count
            elif content == 'global-count':
                global_count += 1
                return global_count
            elif content == 'reset':
                count = 0
                return
            elif content == 'global-reset':
                global_count = 0
                return 
            else:
                return
